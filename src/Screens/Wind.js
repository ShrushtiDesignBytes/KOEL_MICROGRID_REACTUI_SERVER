...
const Wind = ({ BaseUrl }) => {
    ...
    const fetchAlerts = async () => {
        try {
            const response = await fetch(`${BaseUrl}/wind`);
            if (!response.ok) {
    ...
    const displayCounts = (data) => {
        ...
        const alerts = windData.filter((i) => i.severity.toLowerCase() === 'alert');
        const shutdown = windData.filter((i) => i.severity.toLowerCase() === 'shutdown');
        ...
        !loading && <div className="p-4">
            ...
            <div className="grid grid-cols-2 gap-5">
                <div className="relative block">
                    ...
                    />
                    {/* <div className="absolute top-2 left-5 flex items-center max-w-[calc(100%-40px)]">
                        <div className="mr-3">
                ...
                <div className="grid grid-rows-[25%_70%] gap-4">
                    ...
                    <div id="grid-it-rl" className="rounded-lg mt-2 p-4" ref={containerRef}>
                        ...
                        <div className="mt-4 h-[200px] xl:h-[300px]" id="my_dataviz"></div>
                    </div>
                </div>
            </div>
            {/* Second Row Section */}
                            ...
                            <div className="bg-[#030F0E] rounded-lg pb-2.5 overflow-y-auto h-[240px] xl:h-[260px]"
                                ...
                                <table className="w-full border-collapse text-[#CACCCC] text-xs xl:text-sm">
                                    ...
                                    <tbody className="bg-[#030F0E] capitalize text-[#CACCCC]" id="alert-container">
                                        {alertsData.filter(i => i.category === 'wind').reverse().map((item, index) => (
                                            <tr key={index}>
